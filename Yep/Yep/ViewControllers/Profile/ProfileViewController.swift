...
final class ProfileViewController: SegueViewController, CanScrollsToTop {
    ...
    fileprivate lazy var customNavigationBar: UINavigationBar = {
        ...
        if UIScreen.main.bounds.height == YepConfig.iphoneXH {
            y += 20
        }
        
        let bar = UINavigationBar(frame: CGRect(x: 0, y: y, width: self.view.bounds.width, height: 64))
    ...
    fileprivate let skillTextAttributes = [NSAttributedString.Key.font: UIFont.skillTextFont()]

    fileprivate var footerCellHeight: CGFloat {
        let attributes = [
            NSAttributedString.Key.font: YepConfig.Profile.introductionFont
        ]
        let labelWidth = self.collectionViewWidth - (YepConfig.Profile.leftEdgeInset + YepConfig.Profile.rightEdgeInset)
        let rect = self.introductionText.boundingRect(with: CGSize(width: labelWidth, height: CGFloat.greatestFiniteMagnitude), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes:attributes, context:nil)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case .header:
            ...
            cell.updatePrettyColorAction = { [weak self] prettyColor in
                ...
                let textAttributes = [
                    NSAttributedString.Key.foregroundColor: prettyColor,
                    NSAttributedString.Key.font: UIFont.navigationBarTitleFont()
                ]
...
//            return UIEdgeInsets(top: 0, left: sectionLeftEdgeInset, bottom: sectionBottomEdgeInset, right: sectionRightEdgeInset)
    ...
    func collectionView(_ collectionView: UICollectionView!, layout collectionViewLayout: UICollectionViewLayout!, sizeForItemAtIndexPath indexPath: IndexPath!) -> CGSize {
        ...
        case .master:
            ...
            let rect = skillLocalName.boundingRect(with: CGSize(width: CGFloat.greatestFiniteMagnitude, height: SkillCell.height), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: skillTextAttributes, context: nil)
...
extension ProfileViewController {
    ...
    @objc func handleOAuthResult(_ notification: Notification) {
