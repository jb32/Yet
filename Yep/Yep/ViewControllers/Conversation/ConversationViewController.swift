...
final class ConversationViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let moreBarButtonItem = UIBarButtonItem(image: UIImage.yep_iconMore, style: UIBarButtonItem.Style.plain, target: self, action: #selector(ConversationViewController.moreAction(_:)))
        ...
        if let indexOfSearchedMessage = indexOfSearchedMessage {
            let fixedIndexOfSearchedMessage = max(0, indexOfSearchedMessage - Ruler.iPhoneVertical(5, 6, 8, 10, 10, 10).value)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(ConversationViewController.didRecieveMenuWillShowNotification(_:)), name: UIMenuController.willShowMenuNotification, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(ConversationViewController.didRecieveMenuWillHideNotification(_:)), name: UIMenuController.willHideMenuNotification, object: nil)
        ...
        messagesNotificationToken = messages.observe({ [weak self] (change: RealmCollectionChange) in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "showGroupMembers":
            ...
            if let groupId = feedView?.feed?.groupId {
                vc.prepare(groupId)
            }
    ...
    fileprivate func setConversaitonCollectionViewOriginalContentInset() {
        ...
        conversationCollectionView.contentInset.top = 0 + feedViewHeight + conversationCollectionViewContentInsetYOffset
        ...
        case MessageMediaType.text.rawValue:
            ...
            if message.isIndicator {
                ...
                if let _rect = ChatTextCellLayout.textContentTextViewFrameOfMessage(message) {
                    ...
                    rect = message.textContent.boundingRect(with: CGSize(width: messageTextContentTextViewMaxWidth, height: .greatestFiniteMagnitude), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: YepConfig.ChatCell.textAttributes, context: nil)
    ...
    fileprivate func trySendInstantMessageWithType(_ type: YepFayeService.InstantStateType) {
        ...
        let instantMessage: JSONDictionary = [
            ...
            "type": recipient.type.nameForServer,//聊天类型 一对一 或 群聊
            "to": recipient.ID,//消息发送者
            "from": YepUserDefaults.userID.value ?? 0,//消息接受者
        ]
