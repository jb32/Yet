...
final class FeedsViewController: BaseViewController, CanScrollsToTop {
    ...
    weak var parentVC: FeedsContainerViewController?
    ...
    @IBOutlet fileprivate weak var feedsTableView: UITableView!  {
        didSet {
            ...
            feedsTableView.separatorStyle = UITableViewCell.SeparatorStyle.singleLine
    ...
    fileprivate lazy var skillTitleView: UIView = {
        ...
        let textAttributes = [
            ...
            NSAttributedString.Key.font: UIFont.skillHomeTextLargeFont()
        ]
    ...
    fileprivate func updateFeedsTableViewOrInsertWithIndexPaths(_ indexPaths: [IndexPath]?, animation: UITableView.RowAnimation? = nil) {
    ...
    override func viewDidLoad() {
        ...
        navItem.title = String.trans_titleFeeds
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(FeedsViewController.didRecieveMenuWillShowNotification(_:)), name: UIMenuController.willShowMenuNotification, object: nil)

        NotificationCenter.default.addObserver(self, selector: #selector(FeedsViewController.didRecieveMenuWillHideNotification(_:)), name: UIMenuController.willHideMenuNotification, object: nil)
        ...
        if skill != nil {
            navItem.titleView = skillTitleView
            ...
            if let skillID = skill?.id, let me = me() {
                ...
                if notInMaster && me.learningSkills.filter(predicate).count == 0 {
                    ...
                    navItem.rightBarButtonItem = addSkillToMeButton
                }
            }
            ...
            filterBarItem = UIBarButtonItem(title: "", style: UIBarButtonItem.Style.plain, target: self, action: #selector(FeedsViewController.showFilter(_:)))
            navItem.leftBarButtonItem = filterBarItem
        ...
        if hideRightBarItem {
            if profileUser?.isMe ?? false {
                ...
                navItem.rightBarButtonItem = moreBarButtonItem
    ...
    fileprivate func updateFeeds(mode: UpdateFeedsMode = .top, finish: (() -> Void)? = nil) {
        ...
        let completion: ([DiscoveredFeed?]) -> Void = { feeds in
            ...
            let validFeeds = feeds.compactMap({ $0 })
    ...
    fileprivate func prepareConversationViewController(_ vc: ConversationViewController, withDiscoveredFeed feed: DiscoveredFeed, inRealm realm: Realm) {
        ...
        vc.conversationDirtyAction = { [weak self] groupID in
            ...
            groupWithGroupID(groupID: groupID, failureHandler: nil, completion: { [weak self] groupInfo in
                ...
                if let feedInfo = groupInfo["feed"] as? JSONDictionary {
