...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey : Any]? = nil) -> Bool {
        Bugly.start(withAppId: "900059898")
        ...
        _ = delay(0.5) {
            ...
            #endif
            JPUSHService.setup(withOption: launchOptions, appKey: "26c877c9dc41a5aff7aa00db", channel: "AppStore", apsForProduction: apsForProduction)
        }
        ...
        //        let _ = try? AVAudioSession.sharedInstance().setCategory(AVAudioSession.Category.playAndRecord, with: AVAudioSession.CategoryOptions.defaultToSpeaker)
        ...
        if isLogined {
            ...
            // 记录启动通知类型
            if let
                notification = launchOptions?[UIApplication.LaunchOptionsKey.remoteNotification] as? UILocalNotification,
...
//        let callbackSelector = #selector(AppDelegate.tagsAliasCallBack(_:tags:alias:))
        ...
        JPUSHService.setTags(Set(["iOS"]), completion: { (iResCode, targs, seq) in
            app?.tagsAliasCallBack(iResCode, tags: targs ?? Set(), alias: pusherID)
        }, seq: 1)
        JPUSHService.setAlias(pusherID, completion: { (iResCode, targs, seq) in
            app?.tagsAliasCallBack(iResCode, tags: Set(), alias: targs ?? "")
        }, seq: 1)
//        JPUSHService.setTags(Set(["iOS"]), alias: pusherID, callbackSelector: callbackSelector, object: self)
    ...
    func unregisterThirdPartyPush() {
        ...
        JPUSHService.setAlias(nil, completion: nil, seq: 1)
//        JPUSHService.setAlias(nil, callbackSelector: nil, object: nil)

        println("unregisterThirdPartyPush")
    }

    @objc fileprivate func tagsAliasCallBack(_ iResCode: Int, tags: Set<AnyHashable>, alias: String) {
    ...
    fileprivate func customAppearce() {
        ...
        UIBarButtonItem.appearance().setTitleTextAttributes([NSAttributedString.Key.foregroundColor: UIColor.yepTintColor()], for: .normal)
        UIBarButtonItem.appearance().setTitleTextAttributes([NSAttributedString.Key.foregroundColor: UIColor.yepTintColor().withAlphaComponent(0.3)], for: .disabled)
        ...
        let textAttributes: [NSAttributedString.Key: Any] = [
            .foregroundColor: UIColor.yepNavgationBarTitleColor(),
