...
final class YepAudioService: NSObject {
    ...
    @objc func checkRecordTimeout(_ timer: Timer) {
...
//            try AVAudioSession.sharedInstance().setCategory(AVAudioSession.Category.record)
    ...
    func endRecord() {
        ...
        queue.async {
            //AVAudioSession.sharedInstance().setCategory(AVAudioSessionCategoryPlayAndRecord, withOptions: AVAudioSessionCategoryOptions.DefaultToSpeaker,error: nil)
            let _ = try? AVAudioSession.sharedInstance().setActive(false, options: AVAudioSession.SetActiveOptions.notifyOthersOnDeactivation)
        }
...
//                try AVAudioSession.sharedInstance().setCategory(AVAudioSession.Category.playback)
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(YepAudioService.proximityStateChanged), name: UIDevice.proximityStateDidChangeNotification, object: UIDevice.current)
    }
