...
private let fayeQueue = DispatchQueue(label: "RefrainFaye", attributes: [])
...
final class YepFayeService: NSObject {
    ...
    enum MessageType: String {
        ...
        case instant = "ChatStatus"
        case read = "ReadMessage"
        case messageDeleted = "DeleteMessage"
    }
    ...
    fileprivate func subscribeChannel() {
        ...
        fayeQueue.async { [weak self] in
            ...
            self?.fayeClient.subscribeToChannel(personalChannel) { [weak self] data in
                ...
                // Service 消息
                if let messageInfo = info["payload"] as? JSONDictionary {
                ...
                guard let
                    messageTypeString = info["type"] as? String,
                ...
                case .default:
                    ...
                    guard let messageInfo = info["payload"] as? JSONDictionary else {
                ...
                case .instant:
                    ...
                    if let userID = messageInfo["from"] as? String,
                        let state = messageInfo["status"] as? Int {
    ...
    func sendInstantMessage(_ message: JSONDictionary, completion: @escaping (_ success: Bool) -> Void) {
        ...
        fayeQueue.async { [unowned self] in
            ...
            let data: JSONDictionary = [
                "version": "1.0.0",
                "type": MessageType.instant.rawValue,
                "payload": message
            ]
