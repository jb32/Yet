...
final class YepImageCache {
                ...
                ImageCache.default.retrieveImage(forKey: attachmentOriginKey, options: options) { (image, type) -> () in
                    ...
                    if let image = image {
                        ...
                        if sideLength != 0 {
                            ...
                            let originalData = finalImage.jpegData(compressionQuality: 1.0)
                        ...
                        guard let token = YepUserDefaults.v1AccessToken.value else {
                            println("no authorization for download image")
                            return
                        }
                        ...
                        downloadOptions.append(.requestModifier(modifier))
                        
                        ImageDownloader.default.downloadImage(with: url, options: downloadOptions, progressBlock: { receivedSize, totalSize  in
                            ...
                            if let image = image {
                                ...
                                ImageCache.default.store(storeImage,  original: storeImage.jpegData(compressionQuality: 1.0), forKey: attachmentSideLengthKey, toDisk: true, completionHandler: nil)
                    ...
                    if message.attachmentExpiresUnixTime < (Date().timeIntervalSince1970 + (60 * 60 * 24)) {
                        ...
                        refreshAttachmentWithID(message.attachmentID, failureHandler: nil, completion: { newAttachmentInfo in
                            ...
                            if let message = messageWithMessageID(messageID, inRealm: realm) {
                                ...
                                if let attachmentExpiresUnixTime = newAttachmentInfo["expiresAt"] as? TimeInterval {
                                    message.attachmentExpiresUnixTime = attachmentExpiresUnixTime
                                }

                                if let URLString = newAttachmentInfo["url"] as? String {
                                    message.attachmentURLString = URLString
                                }

                                if let URLString = newAttachmentInfo["thumbnail"] as? String {
        ...
        if let image = cache.object(forKey: imageKey) {
            ...
            if let coordinate = message.coordinate {
                ...
                self.cacheQueue.async {
                    ...
                    options.region = MKCoordinateRegion(center: mapCoordinate, latitudinalMeters: 500, longitudinalMeters: 500)
    ...
    func mapImageOfLocationCoordinate(_ locationCoordinate: CLLocationCoordinate2D, withSize size: CGSize, completion: @escaping (UIImage) -> ()) {
        ...
        if let image = cache.object(forKey: imageKey) {
            ...
            let size = size
            options.size = size
            options.region = MKCoordinateRegion(center: locationCoordinate, latitudinalMeters: 500, longitudinalMeters: 500)
