...
final class MessageToolbar: UIToolbar {
    ...
    lazy var micButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(MessageToolbar.toggleRecordVoice), for: UIControl.Event.touchUpInside)
        return button
    }()
    ...
    lazy var moreButton: UIButton = {
        ...
        button.tintColor = UIColor.messageToolBarColor()
        button.tintAdjustmentMode = .normal
        button.addTarget(self, action: #selector(MessageToolbar.moreMessageTypes), for: UIControl.Event.touchUpInside)
        return button
    }()
    ...
    lazy var sendButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(MessageToolbar.trySendTextMessage), for: UIControl.Event.touchUpInside)
        return button
    }()
    ...
    func makeUI() {
        ...
        messageTextViewHeightConstraint.priority = UILayoutPriority.defaultHigh
        ...
        voiceRecordButtonHeightConstraint.priority = UILayoutPriority.defaultHigh
    ...
    func updateHeightOfMessageTextView() {
        ...
        let size = messageTextView.sizeThatFits(CGSize(width: messageTextView.bounds.width, height: CGFloat.greatestFiniteMagnitude))

        let newHeight = size.height
        let limitedNewHeight = min(Ruler.iPhoneVertical(60, 80, 100, 100, 100, 100).value, newHeight)
    ...
    @objc func updateDraft(_ notification: Notification) {
...
extension MessageToolbar: UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if needDetectMention {
            ...
            if text.hasSuffix("@") {
                mentionUsernameRange = text.index(text.endIndex, offsetBy: -1)..<text.endIndex
