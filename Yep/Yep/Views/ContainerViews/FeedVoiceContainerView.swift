...
final class FeedVoiceContainerView: UIView {
    ...
    class func fullWidthWithSampleValuesCount(_ count: Int, timeLengthString: String) -> CGFloat {
        let rect = timeLengthString.boundingRect(with: CGSize(width: 320, height: CGFloat.greatestFiniteMagnitude), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: YepConfig.FeedBasicCell.voiceTimeLengthTextAttributes, context: nil)
    ...
    fileprivate func makeUI() {
        ...
        playButton.setContentHuggingPriority(UILayoutPriority.defaultHigh, for: .horizontal)
        timeLengthLabel.setContentHuggingPriority(UILayoutPriority.defaultHigh, for: .horizontal)
