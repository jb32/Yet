...
final class FeedView: UIView {
    ...
    var feed: ConversationFeed? {
        willSet {
            if let feed = newValue {
                self.configureWithFeed(feed)
                if let groupId = feed.groupId {
                    getCircleMembers(groupId, failureHandler:nil, completion:{ members in
                        self.configureWithMembers(members)
                    })
                }
            }
        }
    ...
    var foldProgress: CGFloat = 0 {
        willSet {
            ...
            UIView.animate(withDuration: 0.25, delay: 0, usingSpringWithDamping: 0.75, initialSpringVelocity: 0.0, options: UIView.AnimationOptions(rawValue: 0), animations: { [weak self] in
    ...
    @IBOutlet weak var memberAvatar5: UIImageView!
    @IBOutlet weak var memberAvatar6: UIImageView!
    @IBOutlet weak var memberAccessory: UIImageView!
    @IBOutlet weak var membersViewTopConstraint: NSLayoutConstraint!
    ...
    override func awakeFromNib() {
        ...
        let tapMembers = UITapGestureRecognizer(target: self, action: #selector(FeedView.tapMembers(_:)))
        membersView.isUserInteractionEnabled = true
        membersView.addGestureRecognizer(tapMembers)
        ...
        memberAccessory.tintColor = UIColor.yepCellAccessoryImageViewTintColor()
    ...
    @objc func toggleFold(_ sender: UITapGestureRecognizer) {
    ...
    @objc func tapMembers(_ sender: UITapGestureRecognizer) {
        tapMembersAction?()
    }
    ...
    var normalHeight: CGFloat {
        ...
        let rect = feed.body.boundingRect(with: CGSize(width: FeedView.messageTextViewMaxWidth, height: CGFloat.greatestFiniteMagnitude), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: YepConfig.FeedView.textAttributes, context: nil)

        var height: CGFloat = ceil(rect.height) + 10 + 40 + 4 + 15 + 17 + 15 + 30
    ...
    fileprivate func calHeightOfMessageTextView() {
        ...
        let rect = messageTextView.text.boundingRect(with: CGSize(width: FeedView.messageTextViewMaxWidth, height: CGFloat.greatestFiniteMagnitude), options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: YepConfig.FeedView.textAttributes, context: nil)
    ...
    fileprivate func configureWithFeed(_ feed: ConversationFeed) {
        ...
        timeLabelTopConstraint.constant = hasAttachment ? (15 + (feed.kind == .audio ? 44 : 80) + 50) : 50
        membersViewTopConstraint.constant = hasAttachment ? (15 + (feed.kind == .audio ? 44 : 80) + 35): 35
    ...
    func configureWithMembers(_ members: [DiscoveredUser]) {
        
        if let creator = members[safe: 0] {
            let plainAvatar = PlainAvatar(avatarURLString: creator.avatarURLString, avatarStyle: nanoAvatarStyle)
            memberAvatar1.navi_setAvatar(plainAvatar, withFadeTransitionDuration: avatarFadeTransitionDuration)
        } else {
            memberAvatar1.image = nil
        }
        
        if let creator = members[safe: 1] {
            let plainAvatar = PlainAvatar(avatarURLString: creator.avatarURLString, avatarStyle: nanoAvatarStyle)
            memberAvatar2.navi_setAvatar(plainAvatar, withFadeTransitionDuration: avatarFadeTransitionDuration)
        } else {
            memberAvatar2.image = nil
        }
        
        if let creator = members[safe: 2] {
            let plainAvatar = PlainAvatar(avatarURLString: creator.avatarURLString, avatarStyle: nanoAvatarStyle)
            memberAvatar3.navi_setAvatar(plainAvatar, withFadeTransitionDuration: avatarFadeTransitionDuration)
        } else {
            memberAvatar3.image = nil
        }
        
        if let creator = members[safe: 3] {
            let plainAvatar = PlainAvatar(avatarURLString: creator.avatarURLString, avatarStyle: nanoAvatarStyle)
            memberAvatar4.navi_setAvatar(plainAvatar, withFadeTransitionDuration: avatarFadeTransitionDuration)
        } else {
            memberAvatar4.image = nil
        }
        
        if let creator = members[safe: 4] {
            let plainAvatar = PlainAvatar(avatarURLString: creator.avatarURLString, avatarStyle: nanoAvatarStyle)
            memberAvatar5.navi_setAvatar(plainAvatar, withFadeTransitionDuration: avatarFadeTransitionDuration)
        } else {
            memberAvatar5.image = nil
        }
        
        if let creator = members[safe: 5] {
            let plainAvatar = PlainAvatar(avatarURLString: creator.avatarURLString, avatarStyle: nanoAvatarStyle)
            memberAvatar6.navi_setAvatar(plainAvatar, withFadeTransitionDuration: avatarFadeTransitionDuration)
        } else {
            memberAvatar6.image = nil
        }
        
        if members.count > 6 {
            memberAccessory.isHidden = false
        } else {
            memberAccessory.isHidden = true
        }
    }

    @objc func tapSocialWork(_ sender: UITapGestureRecognizer) {
    ...
    @objc func tapURLInfo(_ sender: UITapGestureRecognizer) {
    ...
    fileprivate func playOrPauseAudio() {
        ...
        if AVAudioSession.sharedInstance().category == AVAudioSession.Category.record {
            do {
