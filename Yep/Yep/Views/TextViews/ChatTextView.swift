    ...
    func createAttributedStringWithString(_ string: String) -> NSAttributedString {
        ...
        func addMentionAttributes(withRange range: NSRange) {
            ...
            let textAttributes: [NSAttributedString.Key: Any] = [
                .link: textValue,
                NSAttributedString.Key(rawValue: ChatTextView.detectionTypeName): DetectionType.mention.rawValue,
            ]
...
extension ChatTextView: UITextViewDelegate {
    ...
    func textView(_ textView: UITextView, shouldInteractWith URL: URL, in characterRange: NSRange) -> Bool {
        ...
        if let detectionTypeName = self.attributedText?.attribute(.init(ChatTextView.detectionTypeName), at: characterRange.location, effectiveRange: nil) as? String, let detectionType = DetectionType(rawValue: detectionTypeName) {
    ...
    fileprivate func hangleTapText(_ text: String, withDetectionType detectionType: DetectionType) {
        ...
        let username = String(text[text.index(text.startIndex, offsetBy: 1)..<text.endIndex])
        //.substring(from: text.index(text.startIndex, offsetBy: 1))
