        ...
        if let
            ...
            let skillLocalName = skillInfo["name"] as? String {
                ...
                if let userSkill = userSkill {
                    ...
                    if let coverURLString = skillInfo["cover"] as? String {
                        userSkill.coverURLString = coverURLString
                    }
                    ...
                    if let
                        ...
                        let skillCategoryLocalName = categoryData["name"] as? String {
        ...
        realmQueue.async {
            ...
            if let myUserID = YepUserDefaults.userID.value {
                ...
                if me == nil {
                    ...
                    if let createdUnixTime = Date.getTimeInterval(friendInfo["createdAt"]) {
                ...
                if let user = me {
                    ...
                    if let muteMode = friendInfo["muteMode"] as? JSONDictionary {
                        let fromString = muteMode["from"] as! String
                        let toString = muteMode["to"] as! String
                    ...
                    let avatarURLString = friendInfo["avatar"] as? String
                    ...
                    let website = friendInfo["website"] as? JSONDictionary
                    ...
                    let phone = friendInfo["phone"] as? JSONDictionary
                    let areaCode = phone?["areaCode"] as? String
                    YepUserDefaults.areaCode.value = areaCode

                    let mobile = phone?["phoneNumber"] as? String
...
public func syncMyConversations(maxMessageID: String? = nil, afterSynced: (() -> Void)? = nil) {
    ...
    myConversations(maxMessageID: maxMessageID, failureHandler: nil) { result in
        ...
        if let userInfos = result["talkers"] as? [JSONDictionary] {

            let discoveredUsers = userInfos.map({ parseDiscoveredUser($0) }).compactMap({ $0 })
        ...
        if let messageInfos = result["messages"] as? [JSONDictionary] {
            ...
            let messageIDs: [String] = messageInfos.map({ $0["id"] as? String }).compactMap({ $0 })
            messageIDs.forEach({
        ...
        if let lastMessageID =  lastMessageID {
            if let count = result["count"] as? Int, let perPage = result["pagesize"] as? Int {
                if count > perPage {
...
public func syncFriendshipsAndDoFurtherAction(_ furtherAction: @escaping () -> Void) {
    ...
    friendships(failureHandler: nil) { allFriendships in
        ...
        realmQueue.async {
            ...
            for friend in allFriendships {
                if let userID = friend["id"] as? String {
            ...
            for friendshipInfo in allFriendships {
                let friendInfo = friendshipInfo
                
                if let userID = friendshipInfo["id"] as? String {
...
public func syncFeedGroupWithGroupInfo(_ groupInfo: JSONDictionary, inRealm realm: Realm) {
    ...
    if let
        feedInfo = groupInfo["feed"] as? JSONDictionary,
    ...
    if let groupID = groupInfo["id"] as? String {
        ...
        if let group = group {
            ...
            if group.conversation == nil {
                ...
                if let updatedUnixTime = Date.getTimeInterval(groupInfo["updatedAt"]) {
            ...
            if let ownerInfo = groupInfo["owner"] as? JSONDictionary {
                if let ownerID = ownerInfo["id"] as? String {
                    ...
                    if owner == nil {
                        ...
                        if let createdUnixTime = Date.getTimeInterval(ownerInfo["createdAt"]) {
                            newUser.createdUnixTime = createdUnixTime
                        }
                ...
                for memberInfo in remoteMembers {
                    ...
                    if let memberID = memberInfo["id"] as? String {
                        ...
                        if member == nil {
                            ...
                            if let createdUnixTime = Date.getTimeInterval(memberInfo["createdAt"]) {
...
public func recordMessageWithMessageID(_ messageID: String, detailInfo messageInfo: JSONDictionary, inRealm realm: Realm) {
    ...
    let deleted = (messageInfo["revoked"] as? Bool) ?? false
    ...
    if let message = messageWithMessageID(messageID, inRealm: realm) {
        ...
        if let attachments = messageInfo["attachments"] as? [JSONDictionary] {
            ...
            for attachmentInfo in attachments {
                ...
                if let attachmentExpiresUnixTime = Date.getTimeInterval(attachmentInfo["expiresAt"]) {
                    message.attachmentExpiresUnixTime = attachmentExpiresUnixTime
                }

                if let URLString = attachmentInfo["url"] as? String {
                    message.attachmentURLString = URLString
                }

                if let URLString = attachmentInfo["thumbnail"] as? String {
            ...
            if let mediaType = messageInfo["mediaType"] as? String {
...
public func isServiceMessageAndHandleMessageInfo(_ messageInfo: JSONDictionary, inRealm realm: Realm) -> Bool {
    ...
    func tryDeleteGroup(totally: Bool = false) {
        ...
        if let groupID = messageInfo["circle"] as? String, let group = groupWithGroupID(groupID, inRealm: realm) {
    ...
    case .groupCreate:
        ...
        if let groupID = messageInfo["circle"] as? String {
    ...
    case .groupAddUser:
        ...
        guard let userID = actionInfo["target"] as? String else {
            break
        }
...
public func syncMessageWithMessageInfo(_ messageInfo: JSONDictionary, messageAge: MessageAge, inRealm realm: Realm, andDoFurtherAction furtherAction: ((_ messageIDs: [String]) -> Void)?) {
    ...
    if let messageID = messageInfo["id"] as? String {
        ...
        if message == nil {
            ...
            if let createdUnixTime = Date.getTimeInterval(messageInfo["time"]) {
            ...
            if let senderInfo = messageInfo["sender"] as? JSONDictionary {
                if let senderID = senderInfo["id"] as? String {
                    ...
                    if let sender = sender {
                        ...
                        if let recipientType = messageInfo["type"] as? String {
                            ...
                            if recipientType == "Circle" {
                                if let groupID = messageInfo["circle"] as? String {
                                    ...
                                    if sendFromGroup == nil {
                                        ...
                                        if let textContent = messageInfo["content"] as? String, textContent.yep_mentionedMeInRealm(realm) {
                                            syncGroupWithGroupID(groupID)
                                        }
                                    }
                                }
                        ...
                        if let sendFromGroup = sendFromGroup {
                            ...
                            if sender.userID != YepUserDefaults.userID.value {
                                ...
                                guard let userID = messageInfo["sender"] as? String else {
